...
import { Plus, Layout, RotateCcw, ChevronDown } from 'lucide-react';
...
export function DesignToolbar({ tokens, designMode, presets, onOpenAdd, onApplyPreset, onReset }: Props) {
              ...
              <div className="p-1.5">
                {presets.map(p => (
                  ...
                  >
                    ...
                    <div>
                      <p className="text-xs font-semibold" style={{ color: tokens.textPrimary }}>{p.name}</p>
                      <p className="text-[10px]" style={{ color: tokens.textGhost }}>{p.description}</p>
                    </div>
                  </button>
                ))}
              </div>
        ...
        <>
          ...
          >
            <span
              ...
              style={{ backgroundColor: tokens.accent, boxShadow: `0 0 6px ${tokens.accentGlow}`, flexShrink: 0 }}
            />
