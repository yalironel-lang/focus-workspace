...
// Sophisticated palette — no bright candy colors
const ACCENTS = [
  { from: 'from-indigo-500',  to: 'to-violet-600',  bg: 'bg-indigo-600'  },
  { from: 'from-slate-600',   to: 'to-slate-800',   bg: 'bg-slate-700'   },
  { from: 'from-emerald-500', to: 'to-teal-600',    bg: 'bg-emerald-600' },
  { from: 'from-amber-500',   to: 'to-orange-500',  bg: 'bg-amber-500'   },
  { from: 'from-blue-600',    to: 'to-indigo-600',  bg: 'bg-blue-700'    },
  { from: 'from-violet-500',  to: 'to-purple-700',  bg: 'bg-violet-600'  },
];
...
// Display-only rename: "Exercises" shown as "To Do"
...
export function SectionCard({ section, onDelete }: SectionCardProps) {
  ...
  return (
    <div className="group relative bg-white rounded-2xl border border-slate-100 hover:border-slate-200 hover:shadow-lg transition-all duration-200 overflow-hidden flex flex-col">
      {/* Gradient top strip */}
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h3 className="font-semibold text-slate-900 text-sm leading-snug truncate max-w-[180px]">
                {section.title}
              </h3>
        ...
        <div className="flex items-center gap-2 flex-wrap mb-2">
          ...
          {readiness && (
            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${readiness.cls}`}>
              {readiness.label}
            </span>
          )}
        ...
        {section.exam_date && examDays !== null && (
          <div className={`inline-flex items-center gap-1.5 text-[10px] font-semibold px-2 py-0.5 rounded-full mb-2 w-fit ${
            examDays <= 7 ? 'bg-rose-50 text-rose-600' : examDays <= 14 ? 'bg-amber-50 text-amber-700' : 'bg-slate-100 text-slate-500'
          }`}>
        ...
        {section.next_item_title && (
          <p className="text-[11px] text-slate-400 truncate mb-2 leading-snug">
            <span className="font-medium text-slate-500">Next: </span>{section.next_item_title}
          </p>
        )}
        ...
        <div className="mt-auto pt-3 border-t border-slate-50">
          <Link
            ...
            className="flex items-center justify-between w-full text-sm font-semibold text-slate-600 hover:text-primary-600 transition-colors group/link"
          >
